...
(function(){
  var ref$, z, j, l, SI, R, guard, guardjs, noop, moduleName, unfinished, sim, utilInspectCustom, registry, booly, local, checkF, createError, flator, x$, onn, create_atomic, i$, len$, val, key;
  ...
  flator = function(init, inner){
    var ret;
    ret = [":or"];
    if (init.length > 1 && init[0] === ":or") {
      init.shift();
      ret.push.apply(ret, init);
    } else {
      ret.push(init);
    }
    if (inner.length > 1 && inner[0] === ":or") {
      inner.shift();
      ret.push.apply(ret, inner);
    } else {
      ret.push(inner);
    }
    return ret;
  ...
  registry.unit.or = function(data, funs, value){
    ...
    for (i$ = 0, len$ = funs.length; i$ < len$; ++i$) {
      ...
      (ref$ = topRet.path).push.apply(ref$, localRet.path);
    }
    return topRet;
  };
  registry.unit.map.array = function(data, f, value){
    var topRet, val, i$, len$, key, localRet;
    topRet = data.validator(value);
    if (topRet['continue']) {
      val = topRet.value;
  ...
  registry.unit.map.object = function(data, f, UFO){
    var topRet, key, ref$, value, localRet, eR;
    topRet = data.validator(UFO);
    if (topRet['continue']) {
  ...
  registry.basetype.undefined = function(UFO){
    var Type;
    Type = typeof UFO;
    if (Type === 'undefined') {
      return {
